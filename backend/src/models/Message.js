...
async function listInbox(receiverId) {
  const { rows } = await pool.query(
    'SELECT id, receiver_id, text, template, reply, created_at FROM messages WHERE receiver_id = $1 ORDER BY created_at DESC',
    [receiverId]
  );
  return rows;
}
...
module.exports = { createMessage, listInbox, replyToMessage, findById };
