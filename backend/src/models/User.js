...
async function generateUniqueUsername(base = 'user') {
  ...
  if (!(await usernameExists(clean))) return clean;
  let n = 1;
  while (n < 10000) {
...
async function upsertProfile({ id, username, email }) {
  ...
  const query = `
    UPDATE users
    SET username = $2, email = COALESCE($3, email), shareable_link = $4
    WHERE id = $1
...
async function createEmailUser(email) {
  ...
  const { rows } = await pool.query(
    'INSERT INTO users (username, email, shareable_link, is_admin) VALUES ($1,$2,$3,false) RETURNING id, username, email, shareable_link, is_admin',
...
async function findOrCreateGoogleUser({ googleId, email, displayName }) {
  const found = await pool.query('SELECT id, username, email, shareable_link, is_admin FROM users WHERE google_id = $1', [googleId]);
  ...
  const shareableLink = `dua.me/@${username}`;
  const { rows } = await pool.query(
    'INSERT INTO users (username, email, google_id, shareable_link, is_admin) VALUES ($1,$2,$3,$4,false) RETURNING id, username, email, shareable_link, is_admin',
...
async function findByEmail(email) {
  const { rows } = await pool.query('SELECT id, username, email, shareable_link, is_admin FROM users WHERE email = $1', [email]);
  return rows[0];
}
...
async function findById(id) {
  const { rows } = await pool.query('SELECT id, username, email, shareable_link, is_admin FROM users WHERE id = $1', [id]);
  return rows[0];
}
